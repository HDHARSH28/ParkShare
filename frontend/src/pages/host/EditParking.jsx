...
import { getParkingById, updateParking } from '../../services/parkingService';
...
const EditParking = () => {
  ...
  useEffect(() => {
    const fetchExisting = async () => {
      try {
        ...
        const item = res.data?.parking || res.data;
        if (res.success && item) {
          setFormData({
            title: item.title || '',
            description: item.description || '',
            parkingType: item.parkingType || 'Home Driveway',
            ...
            gateAccess: !!item.gateAccess,
            pricePerHour: item.pricePerHour || 0,
            pricePerDay: item.pricePerDay || 0,
            pricePerMonth: item.pricePerMonth || 0,
            status: item.status || 'draft',
            availability: {
              days: item.availability?.days || ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'],
              startTime: item.availability?.startTime || '08:00',
              endTime: item.availability?.endTime || '18:00',
            },
            photos: item.photos || [],
            rules: item.rules || [],
          });
        ...
        console.error('Error fetching parking for edit:', err);
        setError(err.response?.data?.message || 'Failed to load parking spot for editing');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleUseCurrentLocation = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          setError('Could not retrieve GPS location.');
        }
      );
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      const res = await updateParking(id, {
        ...formData,
      ...
      setError(err.response?.data?.message || 'Failed to update parking details');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-surface-700 text-sm mt-1">
          Modify listing details, adjust pricing, or change status
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white rounded-3xl border border-surface-200 p-6 sm:p-8 shadow-sm space-y-4">
          ...
          <div className="flex gap-2">
            <input
              type="text"
              placeholder="e.g. Keep gate closed after entering"
              value={newRule}
